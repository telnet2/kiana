...
describe('VFSMemFS Integration with Real VFS Server (TypeScript)', function() {
  ...
  before(async function() {
    ...
    try {
      ...
      console.error('❌ Failed to load VFS module:', error.message || error);
      throw error;
    }
  });

  describe('VFSMemFS Initialization and Connection', function() {
    it('should initialize VFSMemFS with real VFS client', function() {
      // Get auth token from environment variable or use default
      const authToken = process.env.VFS_AUTH_TOKEN || 'local-system-admin';
      
      console.log(`🔑 VFSMemFS TypeScript test using auth token: ${authToken === 'local-system-admin' ? 'default' : 'from VFS_AUTH_TOKEN env var'}`);
    ...
    it('should connect to VFS server through VFSMemFS', async function() {
      try {
        ...
        console.error('❌ VFSMemFS connection failed:', error.message || error);
        throw error;
      }
    });
  });
  ...
  describe('VFSMemFS File Operations in Sync Mode', () => {
    ...
    it('should write and read files in sync mode (immediate VFS write)', async function() {
    ...
    it('should handle binary files in sync mode', async function() {
    ...
    it('should delete files and sync to VFS', async function() {
  ...
  describe('VFSMemFS Directory Operations', () => {
    it('should create and list directories', async function() {
      ...
      try {
        ...
        console.log(`⚠️ Could not verify directory on VFS: ${error.message || error}`);
      }
    ...
    it('should handle nested file operations', async function() {
      ...
      try {
        ...
        console.log(`⚠️ Could not verify nested file on VFS: ${error.message || error}`);
      }
  ...
  describe('VFSMemFS Flush Mode', () => {
    ...
    it('should buffer writes in flush mode', async function() {
    ...
    it('should flush buffered changes to VFS', async function() {
  ...
  describe('VFSMemFS Statistics and Status', () => {
    ...
    it('should track sync status correctly', function() {
  ...
  afterEach(async function() {
    try {
      ...
      console.error(`⚠️ VFSMemFS cleanup failed:`, error.message || error);
