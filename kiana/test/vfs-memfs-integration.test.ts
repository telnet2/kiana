/**
 * VFSMemFS Integration Test - TypeScript Version
...
import { VFSMemFS, VFSClient, VFSMemFSOptions, VFSMemFSStatistics } from '../src/VFSMemFS';
import { MemNode, MemFile, MemDirectory } from '../src/MemFS';

describe('VFSMemFS Integration with Real VFS Server (TypeScript)', function() {
  let vfsMemFS: VFSMemFS;
  let vfs: VFSClient;
  ...
  before(async function() {
    try {
      ...
      console.log('✅ VFS module loaded successfully for VFSMemFS TypeScript tests');
  ...
  describe('VFSMemFS Initialization and Connection', () => {
    it('should initialize VFSMemFS with real VFS client', () => {
      // Generate random test directory
      const randomNum = Math.floor(Math.random() * 1000000);
      testDirectory = `/temp/vfsmemfs-ts-test-${randomNum}`;

      // Initialize VFSMemFS in sync mode
      const options: VFSMemFSOptions = {
        vfs: vfs,
      ...
      vfsMemFS = new VFSMemFS(options);

      expect(vfsMemFS).to.exist;
      expect(vfsMemFS.getStatistics).to.be.a('function');
      
      const stats: VFSMemFSStatistics = vfsMemFS.getStatistics();
  ...
  describe('VFSMemFS File Operations in Sync Mode', () => {
    ...
    it('should write and read files in sync mode (immediate VFS write)', async () => {
      ...
      const readContent = (fileNode as MemFile).read();
    ...
    it('should handle binary files in sync mode', async () => {
      ...
      // Convert binary data to string for VFSMemFS (which only handles strings)
      ...
      const vfsContent = new TextDecoder().decode(vfsBinaryData as Uint8Array);
  ...
  describe('VFSMemFS Directory Operations', () => {
    it('should create and list directories', async () => {
      ...
      const contents = (dirNode as MemDirectory).listChildren();
    ...
    it('should handle nested file operations', async () => {
      ...
      const readContent = (nestedFileNode as MemFile).read();
      expect(readContent).to.equal(content);
  ...
  describe('VFSMemFS Flush Mode', () => {
    let flushVFSMemFS: VFSMemFS;

    beforeEach(async () => {
      // Create a separate VFSMemFS instance in flush mode
      const flushOptions: VFSMemFSOptions = {
        vfs: vfs,
      ...
      flushVFSMemFS = new VFSMemFS(flushOptions);
    ...
    it('should buffer writes in flush mode', async () => {
      ...
      const stats: VFSMemFSStatistics = flushVFSMemFS.getStatistics();
      ...
      const readContent = (flushFileNode as MemFile).read();
  ...
  describe('VFSMemFS Pre-population', () => {
    it('should pre-populate from existing VFS directory', async function() {
      ...
      const prepopOptions: VFSMemFSOptions = {
        vfs: vfs,
      ...
      const prepopVFSMemFS = new VFSMemFS(prepopOptions);
      ...
      const readContent1 = (file1Node as MemFile).read();
      ...
      const readContent2 = (file2Node as MemFile).read();
      expect(readContent2).to.equal(content2);
      
      console.log(`✅ Pre-populated files accessible through VFSMemFS`);
      
      // Check stats
      const stats: VFSMemFSStatistics = prepopVFSMemFS.getStatistics();
  ...
  describe('VFSMemFS Statistics and Status', () => {
    it('should provide accurate statistics', () => {
      ...
      expect(stats.lastSyncTime).to.satisfy((time: any) => time === undefined || time instanceof Date);
