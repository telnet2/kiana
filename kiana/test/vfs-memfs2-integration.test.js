/**
 * VFSMemFS2 Integration Tests (JavaScript)
...
// We need to import VFSMemFS2 from the compiled output
...
describe('VFSMemFS2 Integration with Real VFS Server', function() {
  ...
  before(async function() {
    try {
      ...
      // Initialize real VFS client with token from environment
      vfs = new VFSClass({
      ...
      // Import VFSMemFS2 from compiled output
      try {
        const vfsMemFS2Module = require('../lib/VFSMemFS2');
        VFSMemFS2 = vfsMemFS2Module.VFSMemFS2;
        VFSMemFS2Error = vfsMemFS2Module.VFSMemFS2Error;
        MemFile2 = vfsMemFS2Module.MemFile2;
        MemDirectory2 = vfsMemFS2Module.MemDirectory2;
        console.log('✅ VFSMemFS2 module loaded successfully');
      } catch (error) {
        console.error('❌ Failed to load VFSMemFS2 module:', error.message);
  ...
  describe('Sync Mode - File Operations', () => {
    ...
    it('should handle binary data correctly', async () => {
      ...
      // Read from memory
      const memBuffer = vfsMemFS.readFile('/binary.bin');
      ...
      expect(Buffer.from(vfsData).equals(binaryData)).to.be.true;
    });
