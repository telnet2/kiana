...
function handleMultipleExports(sourcePatterns: string[], destPath: string, context: CommandContext): string {
...
function handleWildcardExportInternal(pattern: string, destPath: string, context: CommandContext, returnSummary: boolean = true): string[] {
    const exportedFiles: string[] = [];
    ...
    if (!dirNode) {
        if (returnSummary) throw new Error(`No such directory: ${dirPath}`);
        return exportedFiles;
    }
    if (!dirNode.isDirectory()) {
        if (returnSummary) throw new Error(`Not a directory: ${dirPath}`);
        return exportedFiles;
    }
...
function handleSingleExport(sourcePath: string, destPath: string, context: CommandContext): string {
...
function handleWildcardExport(pattern: string, destPath: string, context: CommandContext): string {
    // Get matching files
