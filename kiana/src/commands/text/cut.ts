...
export function cut(context: CommandContext, args: string[], stdin: string | null = null): string {
    ...
    const parseSpecification = (spec: string): (number | [number, number])[] => {
    ...
    const processLine = (line: string, specs: (number | [number, number])[]): string => {
        if (characters) {
