...
export function uniq(context: CommandContext, args: string[], stdin: string | null = null): string {
    ...
    const getComparisonKey = (line: string): string => {
        let key = line;
    ...
    // Count occurrences
    const occurrences = new Map<string, { count: number; originalLine: string }>();
