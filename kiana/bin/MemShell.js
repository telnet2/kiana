...
class MemShell {
    ...
    execWithRedirections(commandTokens, stdin = null, redirections = []) {
        ...
        const hasStderrRedirection = redirections.some(r => r.type === '2>' || r.type === '2>>' || r.type === '&>' || r.type === '>&');
     ...
     */
    execSingleWithStderr(commandTokens, stdin = null, stderr) {
        if (!commandTokens || commandTokens.length === 0) {
            return '';
        }
        const command = commandTokens[0];
     ...
     */
    execForLoop(variable, itemsExpr, loopBody, expandSubstitutions) {
        ...
        for (const item of items) {
            ...
            try {
                ...
                console.error(`Error in for loop iteration (${variable}=${item}):`, e.message);
            }
        }
        return outputs.join('\n');
    }
