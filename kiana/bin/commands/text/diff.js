...
function diff(context, args, stdin) {
    ...
    if (parsed.ignore_blank_lines) {
        const filterBlanks = (lines, norms) => {
            const result = { lines: [], norms: [], map: [] };
 ...
 */
function formatNormalDiff(file1, file2, lines1, lines2, diffResult) {
    const output = [];
    let i = 0;
 ...
 */
function formatUnifiedDiff(file1, file2, lines1, lines2, diffResult, context) {
    const output = [];
    output.push(`--- ${file1}`);
    output.push(`+++ ${file2}`);
    let i = 0;
    while (i < diffResult.length) {
        ...
        for (let k = hunkStart; k < hunkEnd; k++) {
            if (diffResult[k].type === 'common' || diffResult[k].type === 'delete')
                count1++;
            if (diffResult[k].type === 'common' || diffResult[k].type === 'add')
                count2++;
        }
 ...
 */
function formatContextDiff(file1, file2, lines1, lines2, diffResult, context) {
    const output = [];
    output.push(`*** ${file1}`);
    output.push(`--- ${file2}`);
    let i = 0;
    while (i < diffResult.length) {
        ...
        for (let k = hunkStart; k < hunkEnd; k++) {
            if (diffResult[k].line1 !== undefined)
                line1End = diffResult[k].line1 + 1;
            if (diffResult[k].line2 !== undefined)
                line2End = diffResult[k].line2 + 1;
        }
        output.push(`***************`);
