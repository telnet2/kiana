...
function getArkConfig() {
  ...
  const baseURL = process.env.ARK_BASE_URL || 'https://ark-runtime-api.aiheima.com/v1';
  const modelId = process.env.ARK_MODEL_ID || 'gpt-4o-mini';
...
export async function POST(req: NextRequest) {
  ...
  const { sessionId, id, messages, systemPrompt, maxRounds, message } = (body || {}) as {
    sessionId?: string;
    id?: string;
    messages?: UIMessage[];
    message?: string;
  ...
  let inputMessages: UIMessage[] = Array.isArray(messages) ? (messages as any) : [];
  if (!Array.isArray(messages) && typeof message === 'string' && message.trim().length > 0) {
    inputMessages = [
      {
        id: `user-${Date.now()}`,
        role: 'user',
