...
const buildTool = (name: string, doFetch: typeof fetch = fetch): Tool => ({
  name,
  description: "Search the web using the Exa MCP endpoint.",
  parameters: z.object({
...
export const createWebSearchTool = (doFetch: typeof fetch = fetch) => buildTool("websearch", doFetch);
export const createExaWebSearchTool = (doFetch: typeof fetch = fetch) => buildTool("websearch_exa", doFetch);
